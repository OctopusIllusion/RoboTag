...
#pragma config(Sensor, S2,     frontTouch,         sensorTouch)
...
task Wall_Follow()
{
	...
	if(true) {
		...
		SetBothMotors(slow_driving_speed);
	...
	while(true)
		{
			...
			// robot is "it"
		  	...
		  	// probably should change to either front or back sensor right?
				...
				// Back Touch Sensor Triggered
				// nulling this block of code for now, return it later
				/*
				else if(SensorValue(backTouch) == 1) {
					//spin_head();
			  	StopTask(Drive_Forward);
			  	//StopTask(Sound_Watch);
			  	motor(rightServo) = 0;
 					motor(leftServo) = 0;
 					ring_bell();
					//speak_loudly();
					...
					// drive forward a little to get away from what hit us
					motor(rightServo) = driving_speed;
 					motor(leftServo) = driving_speed;
 					wait1Msec(700);
 					motor(rightServo) = 0;
 					motor(leftServo) = 0;

 					// listen here...
 					if(SensorValue[soundSense] > soundThreshold) {
 						// if we hear something now - it's the other robot! We tagged it
 					  wait1Msec(300);
