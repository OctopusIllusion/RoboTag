#pragma config(Sensor, S2,     soundFront,				sensorSoundDB)
#pragma config(Sensor, S3,     backTouch,         sensorTouch)
#pragma config(Sensor, S4,     soundBack,     sensorSoundDB)
#pragma config(Sensor, S1,     frontTouch,     sensorTouch)
...
// ends up with robot facing some obstacle - so most likely to be bumpbed in Back Touch Sensor
void hide_somewhere() {
...
task Bump_Watch()
{
	...
	// behavioral_state
	// 1 means "it" - Robin is "it" and must pursue other robots
	// 2 means "not it" - Robin is "not it" and must run/hide to avoid being tagged by the "it" robot
	...
	while(true)
        ...
        //////////////////////////////////////////////
		  	...
		  	// Something presses Back Sensor
				//////////////////////////////////////////////
				// if bump is struck while there is a loud noise - could be a fellow robot!
		  	else if(SensorValue(backTouch) == 1)  {
		  	  wait1Msec(200);
...
// Not applicable - no sonar sensor on Robin
...
task Sonar_Watch()
{
		...
		while(true)
		{
			if(SensorValue[frontSonar] <= distanceThreshold){
				//StopTask(Drive_Forward);
				//motor(rightServo) = 0;
 				//motor(leftServo) = 0;
